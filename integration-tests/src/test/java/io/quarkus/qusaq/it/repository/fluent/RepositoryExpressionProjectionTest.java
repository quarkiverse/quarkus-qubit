 ...
 * Mirrors io.quarkus.qusaq.it.fluent.ExpressionProjectionTest using repository injection.
 *
 ...
 * <p>Examples:
 * <ul>
 * <li>Arithmetic: {@code repository.select(p -> p.salary * 1.1).toList()}</li>
 * <li>String concat: {@code repository.select(p -> p.firstName + " " + p.lastName).toList()}</li>
 * <li>Combined: {@code repository.where(p -> p.active).select(p -> p.salary * 1.1).toList()}</li>
 * </ul>
 */
@QuarkusTest
class RepositoryExpressionProjectionTest {
    ...
    void selectArithmetic_integerAddition() {
        var agesPlus5 = personRepository.select((Person p) -> p.age + 5).toList();
    ...
    void selectArithmetic_doubleAddition() {
        var salaryIncrease = personRepository.select((Person p) -> p.salary + 5000.0).toList();
    ...
    void selectArithmetic_integerSubtraction() {
        var agesMinus2 = personRepository.select((Person p) -> p.age - 2).toList();
    ...
    void selectArithmetic_doubleSubtraction() {
        var salaryDecrease = personRepository.select((Person p) -> p.salary - 10000.0).toList();
    ...
    void selectArithmetic_integerMultiplication() {
        var agesDoubled = personRepository.select((Person p) -> p.age * 2).toList();
    ...
    void selectArithmetic_doubleMultiplication_tenPercentRaise() {
        var salariesRaised = personRepository.select((Person p) -> p.salary * 1.1).toList();
    ...
    void selectArithmetic_floatMultiplication() {
        var heightsInCm = personRepository.select((Person p) -> p.height * 100.0f).toList();
    ...
    void selectArithmetic_integerDivision() {
        var agesHalved = personRepository.select((Person p) -> p.age / 2).toList();
    ...
    void selectArithmetic_doubleDivision() {
        var monthlySalaries = personRepository.select((Person p) -> p.salary / 12.0).toList();
    ...
    void selectArithmetic_integerModulo() {
        var agesMod10 = personRepository.select((Person p) -> p.age % 10).toList();
    ...
    void selectStringConcat_fullName() {
        var fullNames = personRepository.select((Person p) -> p.firstName + " " + p.lastName).toList();
    ...
    void selectStringConcat_emailPrefix() {
        var greetings = personRepository.select((Person p) -> "Hello, " + p.firstName).toList();
    ...
    void selectStringConcat_multipleStrings() {
        var formattedNames = personRepository.select((Person p) ->
    ...
    void whereActive_selectSalaryRaise() {
        var raisedSalaries = personRepository.where((Person p) -> p.active)
    ...
    void whereAgeLessThan30_selectFullName() {
        var youngPeopleNames = personRepository.where((Person p) -> p.age < 30)
    ...
    void whereSalaryGreaterThan70K_selectAgeDoubled() {
        var highEarnersAges = personRepository.where((Person p) -> p.salary > 70000.0)
    ...
    void selectComplexArithmetic_multipleOperations() {
        // Calculate: (age * 2) + 10
        var complexAges = personRepository.select((Person p) -> (p.age * 2) + 10).toList();
    ...
    void selectComplexArithmetic_salaryCalculation() {
        ...
        var bonuses = personRepository.select((Person p) -> (p.salary * 0.1) + 5000.0).toList();
    ...
    void productSelect_priceWithTax() {
        var pricesWithTax = productRepository.select((Product p) -> p.stockQuantity * 2).toList();
    ...
    void productWhereAvailable_selectDiscountedStock() {
        var discountedStock = productRepository.where((Product p) -> p.available)
    ...
    void productSelect_nameWithCategory() {
        var productDescriptions = productRepository.select((Product p) ->
                p.name + " [" + p.category + "]"
        ).toList();
    ...
    void selectExpression_resultCountMatchesEntityCount() {
        ...
        var expressions = personRepository.select((Person p) -> p.age + 10).toList();
    ...
    void selectArithmetic_withZero() {
        var results = personRepository.select((Person p) -> p.age * 0).toList();
    ...
    void selectArithmetic_divisionResultsInDecimal() {
        var results = personRepository.select((Person p) -> p.age / 3).toList();
