...
class WhereSelectTest {
    ...
    void whereActive_selectEmail() {
        List<String> emails = Person.where((Person p) -> p.active)
    ...
    void whereSalary_selectLastName() {
        List<String> lastNames = Person.where((Person p) -> p.salary > 70000.0)
    ...
    void whereAge_selectEmployeeId() {
        List<Long> employeeIds = Person.where((Person p) -> p.age >= 30)
    ...
    void whereActive_selectSalary() {
        List<Double> salaries = Person.where((Person p) -> p.active)
    ...
    void whereAge_selectHeight() {
        List<Float> heights = Person.where((Person p) -> p.age < 30)
    ...
    void whereAge_selectActive() {
        List<Boolean> activeStatuses = Person.where((Person p) -> p.age > 40)
    ...
    void complexWhere_selectFirstName() {
        List<String> names = Person.where((Person p) -> p.age > 25 && p.active && p.salary > 60000.0)
                                   .select((Person p) -> p.firstName)
                                   .toList();
    ...
    void whereOr_selectEmail() {
        List<String> emails = Person.where((Person p) -> p.age < 26 || p.age > 40)
                                    .select((Person p) -> p.email)
                                    .toList();
    ...
    void whereStringContains_selectAge() {
        List<Integer> ages = Person.where((Person p) -> p.email.contains("@example.com"))
                                   .select((Person p) -> p.age)
                                   .toList();
    ...
    void whereProductAvailable_selectName() {
        List<String> productNames = Product.where((Product p) -> p.available)
    ...
    void whereProductPrice_selectStockQuantity() {
        List<Integer> stockQuantities = Product.where((Product p) -> p.price.compareTo(new BigDecimal("500.00")) > 0)
    ...
    void whereProductCategory_selectPrice() {
        List<BigDecimal> prices = Product.where((Product p) -> p.category.equals("Electronics"))
    ...
    void whereNoMatches_selectField_returnsEmptyList() {
        List<String> names = Person.where((Person p) -> p.age > 100)
                                   .select((Person p) -> p.firstName)
                                   .toList();
    ...
    void whereAllMatch_selectField_returnsAllProjections() {
        List<Integer> ages = Person.where((Person p) -> p.age > 0)
                                   .select((Person p) -> p.age)
                                   .toList();
