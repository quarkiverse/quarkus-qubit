...
import io.quarkus.qusaq.it.dto.PersonBasicDTO;
import io.quarkus.qusaq.it.dto.PersonNameDTO;
import io.quarkus.qusaq.it.dto.PersonSummaryDTO;
import io.quarkus.qusaq.it.dto.ProductInfoDTO;
 ...
 * Examples:
 * <ul>
 * <li>Simple DTO: {@code Person.select(p -> new PersonNameDTO(p.firstName, p.lastName)).toList()}</li>
 * <li>Multi-type DTO: {@code Person.select(p -> new PersonSummaryDTO(p.firstName, p.age, p.salary)).toList()}</li>
 * <li>Combined: {@code Person.where(p -> p.active).select(p -> new PersonNameDTO(p.firstName, p.lastName)).toList()}</li>
 * </ul>
 */
@QuarkusTest
class DtoProjectionTest {
    ...
    void selectDTO_twoFields() {
        var names = Person.select((Person p) -> new PersonNameDTO(p.firstName, p.lastName)).toList();
    ...
    void selectDTO_threeStringFields() {
        var basics = Person.select((Person p) -> new PersonBasicDTO(p.firstName, p.lastName, p.email)).toList();
    ...
    void selectDTO_multipleTypes() {
        var summaries = Person.select((Person p) -> new PersonSummaryDTO(p.firstName, p.age, p.salary)).toList();
    ...
    void selectDTO_productWithBigDecimal() {
        var products = Product.select((Product p) -> new ProductInfoDTO(p.name, p.price, p.category)).toList();
    ...
    void whereActive_selectDTO() {
        var activeNames = Person.where((Person p) -> p.active)
    ...
    void whereAgeLessThan30_selectDTO() {
        var youngPeople = Person.where((Person p) -> p.age < 30)
    ...
    void whereSalaryGreaterThan70K_selectDTO() {
        var highEarners = Person.where((Person p) -> p.salary > 70000.0)
    ...
    void productWhereAvailable_selectDTO() {
        var availableProducts = Product.where((Product p) -> p.available)
    ...
    void whereActiveAndSalaryGreaterThan60K_selectDTO() {
        var activeHighEarners = Person.where((Person p) -> p.active && p.salary > 60000.0)
                                      .select((Person p) -> new PersonSummaryDTO(p.firstName, p.age, p.salary))
                                      .toList();
    ...
    void productWherePriceGreaterThan300_selectDTO() {
        var expensiveProducts = Product.where((Product p) -> p.price.compareTo(new BigDecimal("300.00")) > 0)
                                       .select((Product p) -> new ProductInfoDTO(p.name, p.price, p.category))
                                       .toList();
    ...
    void selectDTO_resultCountMatchesEntityCount() {
        ...
        var dtos = Person.select((Person p) -> new PersonNameDTO(p.firstName, p.lastName)).toList();
