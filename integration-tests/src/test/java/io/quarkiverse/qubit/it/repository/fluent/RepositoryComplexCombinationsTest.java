...
/**
 * Repository pattern comprehensive test suite for complex operation combinations.
 * Mirrors io.quarkiverse.qubit.it.fluent.ComplexCombinationsTest using repository injection.
 * <p>
 ...
 */
@QuarkusTest
class RepositoryComplexCombinationsTest {
    ...
    void skip_count_ignoresSkipForCount() {
        ...
        long countWithSkip = repository.where((Person p) -> p.age > 0).skip(2).count();
        long countWithoutSkip = repository.where((Person p) -> p.age > 0).count();
    ...
    void sortedDescendingBy_limit_descendingWithLimit() {
        List<Person> results = repository.sortedDescendingBy((Person p) -> p.age)
                .limit(3)
                .toList();
    ...
    void sortedDescendingBy_skip_descendingWithSkip() {
        ...
        List<Person> allSorted = repository.sortedDescendingBy((Person p) -> p.age).toList();

        // Skip the first 2 oldest persons
        List<Person> skipped = repository.sortedDescendingBy((Person p) -> p.age)
                .skip(2)
                .toList();
    ...
    void where_select_limit_filterProjectLimit() {
        List<String> names = repository.where((Person p) -> p.age > 25)
    ...
    void select_sortedBy_skip_projectSortSkip() {
        // Get all ages sorted
        List<Integer> allAges = repository.select((Person p) -> p.age)
                .sortedBy((Integer age) -> age)
                .toList();

        // Project to ages, sort, and skip first 2
        List<Integer> skippedAges = repository.select((Person p) -> p.age)
                .sortedBy((Integer age) -> age)
                .skip(2)
                .toList();
    ...
    void where_sortedBy_skip_filterSortSkip() {
        ...
        List<Person> allActive = repository.where((Person p) -> p.active)
                .sortedBy((Person p) -> p.age)
                .toList();

        // Filter, sort, and skip first active person
        List<Person> skipped = repository.where((Person p) -> p.active)
                .sortedBy((Person p) -> p.age)
                .skip(1)
                .toList();
    ...
    void where_select_sortedBy_limit_fullComplexPipeline() {
        ...
        List<String> names = repository.where((Person p) -> p.active)
                .select((Person p) -> p.firstName)
    ...
    void where_sortedBy_limit_skip_paginationWithFilter() {
        ...
        List<Person> page2 = repository.where((Person p) -> p.active)
