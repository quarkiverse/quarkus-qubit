...
/**
 * Repository pattern comprehensive test suite for distinct() operation combinations.
 * Mirrors io.quarkiverse.qubit.it.fluent.DistinctOperationsTest using repository injection.
 * <p>
 ...
 */
@QuarkusTest
class RepositoryDistinctOperationsTest {
    ...
    void distinct_count_countsUniqueValues() {
        ...
        List<String> distinctLastNames = repository.select((Person p) -> p.lastName)
                .distinct()
                .toList();
    ...
    void where_distinct_filtersAndRemovesDuplicates() {
        ...
        List<String> distinctLastNames = repository.where((Person p) -> p.active)
    ...
    void where_distinct_multiplePredicates() {
        ...
        List<String> distinctNames = repository.where((Person p) -> p.age > 25)
    ...
    void where_distinct_count_filtersAndCountsUnique() {
        long count = repository.where((Person p) -> p.active)
                .select((Person p) -> p.lastName)
                .distinct()
                .count();
    ...
    void where_distinct_count_complexPredicate() {
        long count = repository.where((Person p) -> p.age >= 30)
    ...
    void select_distinct_count_countsUniqueProjectedValues() {
        ...
        // Count distinct ages
        long distinctAgeCount = repository.select((Person p) -> p.age)
                .distinct()
                .count();
    ...
    void distinct_limit_limitsUniqueResults() {
        List<String> limitedDistinct = repository.select((Person p) -> p.lastName)
                .distinct()
                .limit(3)
                .toList();
    ...
    void distinct_skip_skipsUniqueResults() {
        ...
        List<String> allDistinct = repository.select((Person p) -> p.lastName)
                .distinct()
                .toList();

        // Skip first 2 unique values
        List<String> skipped = repository.select((Person p) -> p.lastName)
                .distinct()
                .skip(2)
                .toList();
    ...
    void where_select_distinct_fullPipeline() {
        List<String> results = repository.where((Person p) -> p.active)
                .select((Person p) -> p.lastName)
                .distinct()
                .toList();
    ...
    void where_select_distinct_complexFilter() {
        List<Integer> ages = repository.where((Person p) -> p.salary > 60000.0)
    ...
    void where_distinct_limit_filterLimitUnique() {
        List<String> results = repository.where((Person p) -> p.age > 25)
                .select((Person p) -> p.firstName)
                .distinct()
                .limit(2)
                .toList();
    ...
    void select_distinct_limit_projectLimitUnique() {
        List<String> names = repository.select((Person p) -> p.lastName)
                .distinct()
                .limit(3)
                .toList();
    ...
    void where_select_distinct_limit_fullComplexPipeline() {
        List<Integer> ages = repository.where((Person p) -> p.active)
                .select((Person p) -> p.age)
                .distinct()
                .limit(3)
                .toList();
