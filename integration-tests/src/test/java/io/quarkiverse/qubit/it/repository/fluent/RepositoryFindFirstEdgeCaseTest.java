...
/**
 * Repository pattern tests for findFirst() edge cases verifying return logic correctness.
 * Mirrors io.quarkiverse.qubit.it.fluent.FindFirstEdgeCaseTest using repository injection.
 *
 * <p>Tests that isEmpty() check is necessary even with limit(1) optimization.
 */
@QuarkusTest
class RepositoryFindFirstEdgeCaseTest {
    ...
    void findFirst_noMatches_returnsEmptyOptional() {
        ...
        Optional<Person> result = personRepository.where((Person p) -> p.age > 1000)
                .findFirst();
    ...
    void findFirst_manyMatches_returnsOptionalOfFirstElementOnly() {
        ...
        List<Person> allActive = personRepository.where((Person p) -> p.active).toList();
