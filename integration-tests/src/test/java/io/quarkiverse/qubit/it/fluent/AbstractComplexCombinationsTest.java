 ...
 * <li>sortedDescendingBy + skip → toList (1 test)</li>
 * <li>where + select + limit → toList (1 test)</li>
 * <li>select + sortedBy + skip → toList (1 test)</li>
 * <li>where + sortedBy + skip → toList (1 test)</li>
 * <li>where + select + sortedBy + limit → toList (1 test)</li>
 * <li>where + sortedBy + limit + skip → toList (1 test)</li>
 * </ul>
 */
public abstract class AbstractComplexCombinationsTest {
    ...
    void skip_count_ignoresSkipForCount() {
        ...
        long countWithSkip = personOps().where((Person p) -> p.age > 0).skip(2).count();
        long countWithoutSkip = personOps().where((Person p) -> p.age > 0).count();
    ...
    void sortedDescendingBy_limit_descendingWithLimit() {
        List<Person> results = personOps().sortedDescendingBy((Person p) -> p.age)
                .limit(3)
                .toList();
    ...
    void sortedDescendingBy_skip_descendingWithSkip() {
        ...
        List<Person> allSorted = personOps().sortedDescendingBy((Person p) -> p.age).toList();

        // Skip the first 2 oldest persons
        List<Person> skipped = personOps().sortedDescendingBy((Person p) -> p.age)
                .skip(2)
                .toList();
    ...
    void select_sortedBy_skip_projectSortSkip() {
        // Get all ages sorted
        List<Integer> allAges = personOps().select((Person p) -> p.age)
                .sortedBy((Integer age) -> age)
                .toList();

        // Project to ages, sort, and skip first 2
        List<Integer> skippedAges = personOps().select((Person p) -> p.age)
                .sortedBy((Integer age) -> age)
                .skip(2)
                .toList();
    ...
    void where_sortedBy_skip_filterSortSkip() {
        ...
        List<Person> skipped = personOps().where((Person p) -> p.active)
                .sortedBy((Person p) -> p.age)
                .skip(1)
                .toList();
    ...
    void where_select_sortedBy_limit_fullComplexPipeline() {
        ...
        List<String> names = personOps().where((Person p) -> p.active)
                .select((Person p) -> p.firstName)
    ...
    void where_sortedBy_limit_skip_paginationWithFilter() {
        ...
        List<Person> page2 = personOps().where((Person p) -> p.active)
