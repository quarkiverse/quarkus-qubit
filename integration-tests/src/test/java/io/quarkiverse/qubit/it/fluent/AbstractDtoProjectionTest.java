...
public abstract class AbstractDtoProjectionTest {
    ...
    void selectDTO_threeStringFields() {
        var basics = personOps().select((Person p) -> new PersonBasicDTO(p.firstName, p.lastName, p.email)).toList();
    ...
    void selectDTO_multipleTypes() {
        var summaries = personOps().select((Person p) -> new PersonSummaryDTO(p.firstName, p.age, p.salary)).toList();
    ...
    void selectDTO_resultCountMatchesEntityCount() {
        ...
        var dtos = personOps().select((Person p) -> new PersonNameDTO(p.firstName, p.lastName)).toList();
