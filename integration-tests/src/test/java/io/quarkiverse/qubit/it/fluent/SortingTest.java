 ...
 * - Multi-level sorting
 * - Sorting with filtering (where)
 * - Sorting with projection (select)
...
class SortingTest {
    ...
    void sortedDescendingBy_salary_descendingOrder() {
        List<Person> sorted = Person.sortedDescendingBy((Person p) -> p.salary).toList();
    ...
    void sortedDescendingBy_firstName_reverseAlphabetical() {
        List<Person> sorted = Person.sortedDescendingBy((Person p) -> p.firstName).toList();
    ...
    void multiLevelSort_lastNameThenFirstName() {
        ...
        List<Person> sorted = Person.sortedBy((Person p) -> p.firstName)
    ...
    void multiLevelSort_mixedAscDesc() {
        ...
        List<Person> sorted = Person.sortedDescendingBy((Person p) -> p.salary)
                                    .sortedBy((Person p) -> p.age)
                                    .toList();
    ...
    void whereAndSortedBy_filterAndSort() {
        List<Person> results = Person.where((Person p) -> p.age > 30)
                                     .sortedBy((Person p) -> p.age)
                                     .toList();
    ...
    void selectAndMultiSort_projectWithMultiLevelSort() {
        List<Double> salaries = Person.select((Person p) -> p.salary)
    ...
    void whereSelectSorted_fullPipeline() {
        List<String> names = Person.where((Person p) -> p.age > 30)
    ...
    void sortedBy_productPrice_ascendingBigDecimal() {
        List<Product> sorted = Product.sortedBy((Product p) -> p.price).toList();
    ...
    void sortedDescendingBy_productStockQuantity_descendingInteger() {
        List<Product> sorted = Product.sortedDescendingBy((Product p) -> p.stockQuantity).toList();
