...
class ExpressionProjectionTest {
    ...
    void selectArithmetic_integerAddition() {
        var agesPlus5 = Person.select((Person p) -> p.age + 5).toList();
    ...
    void selectArithmetic_doubleAddition() {
        var salaryIncrease = Person.select((Person p) -> p.salary + 5000.0).toList();
    ...
    void selectArithmetic_integerSubtraction() {
        var agesMinus2 = Person.select((Person p) -> p.age - 2).toList();
    ...
    void selectArithmetic_doubleSubtraction() {
        var salaryDecrease = Person.select((Person p) -> p.salary - 10000.0).toList();
    ...
    void selectArithmetic_integerMultiplication() {
        var agesDoubled = Person.select((Person p) -> p.age * 2).toList();
    ...
    void selectArithmetic_doubleMultiplication_tenPercentRaise() {
        var salariesRaised = Person.select((Person p) -> p.salary * 1.1).toList();
    ...
    void selectArithmetic_floatMultiplication() {
        var heightsInCm = Person.select((Person p) -> p.height * 100.0f).toList();
    ...
    void selectArithmetic_integerDivision() {
        var agesHalved = Person.select((Person p) -> p.age / 2).toList();
    ...
    void selectArithmetic_doubleDivision() {
        var monthlySalaries = Person.select((Person p) -> p.salary / 12.0).toList();
    ...
    void selectArithmetic_integerModulo() {
        var agesMod10 = Person.select((Person p) -> p.age % 10).toList();
    ...
    void selectStringConcat_fullName() {
        var fullNames = Person.select((Person p) -> p.firstName + " " + p.lastName).toList();
    ...
    void selectStringConcat_emailPrefix() {
        var greetings = Person.select((Person p) -> "Hello, " + p.firstName).toList();
    ...
    void selectStringConcat_multipleStrings() {
        var formattedNames = Person.select((Person p) ->
    ...
    void whereActive_selectSalaryRaise() {
        var raisedSalaries = Person.where((Person p) -> p.active)
    ...
    void whereAgeLessThan30_selectFullName() {
        var youngPeopleNames = Person.where((Person p) -> p.age < 30)
    ...
    void whereSalaryGreaterThan70K_selectAgeDoubled() {
        var highEarnersAges = Person.where((Person p) -> p.salary > 70000.0)
    ...
    void whereInactive_selectFullNameWithStatus() {
        var inactivePeople = Person.where((Person p) -> !p.active)
    ...
    void selectComplexArithmetic_multipleOperations() {
        // Calculate: (age * 2) + 10
        var complexAges = Person.select((Person p) -> (p.age * 2) + 10).toList();
    ...
    void selectComplexArithmetic_salaryCalculation() {
        ...
        var bonuses = Person.select((Person p) -> (p.salary * 0.1) + 5000.0).toList();
    ...
    void productSelect_priceWithTax() {
        var pricesWithTax = Product.select((Product p) -> p.stockQuantity * 2).toList();
    ...
    void productWhereAvailable_selectDiscountedStock() {
        var discountedStock = Product.where((Product p) -> p.available)
    ...
    void productSelect_nameWithCategory() {
        var productDescriptions = Product.select((Product p) ->
                p.name + " [" + p.category + "]"
        ).toList();
    ...
    void whereNoMatches_selectExpression_returnsEmptyList() {
        var results = Person.where((Person p) -> p.age > 100)
                            .select((Person p) -> p.age * 2)
                            .toList();
    ...
    void selectExpression_resultCountMatchesEntityCount() {
        ...
        var expressions = Person.select((Person p) -> p.age + 10).toList();
    ...
    void selectArithmetic_withZero() {
        var results = Person.select((Person p) -> p.age * 0).toList();
    ...
    void selectArithmetic_divisionResultsInDecimal() {
        var results = Person.select((Person p) -> p.age / 3).toList();
