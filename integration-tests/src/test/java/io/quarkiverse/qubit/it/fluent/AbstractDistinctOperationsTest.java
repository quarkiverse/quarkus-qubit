 ...
 * <li>where + distinct → count (2 tests)</li>
 * <li>select + distinct → count (1 test)</li>
 * <li>distinct + limit → toList (1 test)</li>
 * <li>distinct + skip → toList (1 test)</li>
 * <li>where + select + distinct → toList (2 tests)</li>
 * <li>where + distinct + limit → toList (1 test)</li>
 * <li>select + distinct + limit → toList (1 test)</li>
 * <li>where + select + distinct + limit → toList (1 test)</li>
 * </ul>
 */
public abstract class AbstractDistinctOperationsTest {
    ...
    @Test
    @Transactional
    protected void distinct_count_countsUniqueValues() {
        ...
        List<String> distinctLastNames = personOps().select((Person p) -> p.lastName)
                .distinct()
                .toList();
    ...
    @Test
    @Transactional
    protected void where_distinct_filtersAndRemovesDuplicates() {
        ...
        List<String> distinctLastNames = personOps().where((Person p) -> p.active)
    ...
    void where_distinct_multiplePredicates() {
        ...
        List<String> distinctNames = personOps().where((Person p) -> p.age > 25)
    ...
    void where_distinct_count_filtersAndCountsUnique() {
        long count = personOps().where((Person p) -> p.active)
                .select((Person p) -> p.lastName)
                .distinct()
                .count();
    ...
    void where_distinct_count_complexPredicate() {
        long count = personOps().where((Person p) -> p.age >= 30)
    ...
    @Test
    @Transactional
    protected void select_distinct_count_countsUniqueProjectedValues() {
        ...
        // Count distinct ages
        long distinctAgeCount = personOps().select((Person p) -> p.age)
                .distinct()
                .count();
    ...
    void distinct_limit_limitsUniqueResults() {
        List<String> limitedDistinct = personOps().select((Person p) -> p.lastName)
                .distinct()
                .limit(3)
                .toList();
    ...
    void distinct_skip_skipsUniqueResults() {
        ...
        List<String> allDistinct = personOps().select((Person p) -> p.lastName)
                .distinct()
                .toList();

        // Skip first 2 unique values
        List<String> skipped = personOps().select((Person p) -> p.lastName)
                .distinct()
                .skip(2)
                .toList();
    ...
    void where_select_distinct_fullPipeline() {
        List<String> results = personOps().where((Person p) -> p.active)
                .select((Person p) -> p.lastName)
                .distinct()
                .toList();
    ...
    void where_select_distinct_complexFilter() {
        List<Integer> ages = personOps().where((Person p) -> p.salary > 60000.0)
    ...
    void where_distinct_limit_filterLimitUnique() {
        List<String> results = personOps().where((Person p) -> p.age > 25)
                .select((Person p) -> p.firstName)
                .distinct()
                .limit(2)
                .toList();
    ...
    void select_distinct_limit_projectLimitUnique() {
        List<String> names = personOps().select((Person p) -> p.lastName)
                .distinct()
                .limit(3)
                .toList();
