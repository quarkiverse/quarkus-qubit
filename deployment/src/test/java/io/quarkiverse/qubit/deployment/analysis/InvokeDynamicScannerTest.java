 ...
 *
 * <p>These tests verify that the scanner correctly identifies lambda call sites
 * in compiled bytecode containing QubitEntity fluent API patterns.
 */
class InvokeDynamicScannerTest {

    private static final String FLUENT_API_SOURCES_CLASS =
            "io.quarkiverse.qubit.deployment.testutil.FluentApiTestSources";
    private static final String FLUENT_API_SOURCES_FILE =
            FLUENT_API_SOURCES_CLASS.replace('.', '/') + ".class";
    ...
    class ScanClass {
        ...
        void scansClassAndFindsCallSites() {
            List<InvokeDynamicScanner.LambdaCallSite> callSites =
                    scanner.scanClass(fluentApiSourcesBytes, FLUENT_API_SOURCES_CLASS);
        ...
        void returnsEmptyListForNullBytes() {
            List<InvokeDynamicScanner.LambdaCallSite> callSites =
                    scanner.scanClass(null, "SomeClass");
        ...
        void returnsEmptyListForInvalidBytecode() {
            ...
            List<InvokeDynamicScanner.LambdaCallSite> callSites =
                    scanner.scanClass(invalidBytecode, "InvalidClass");
        ...
        void returnsEmptyListForClassWithoutLambdas() throws Exception {
            ...
            try (InputStream is = InvokeDynamicScannerTest.class.getClassLoader()
                ...
                if (is != null) {
                    ...
                    List<InvokeDynamicScanner.LambdaCallSite> callSites =
                            scanner.scanClass(classBytes, className);
    ...
    class JoinTypeEnum {
        ...
        @Test
        void innerJoinTypeExists() {
            assertThat(InvokeDynamicScanner.JoinType.INNER).isNotNull();
        }

        @Test
        void leftJoinTypeExists() {
            assertThat(InvokeDynamicScanner.JoinType.LEFT).isNotNull();
        }

        @Test
        void joinTypeValuesAreComplete() {
            assertThat(InvokeDynamicScanner.JoinType.values())
                    .hasSize(2)
                    .containsExactlyInAnyOrder(
                            InvokeDynamicScanner.JoinType.INNER,
                            InvokeDynamicScanner.JoinType.LEFT);
        }
    }

    @Nested
    class QueryContextEnum {

        @Test
        void standardContextExists() {
            assertThat(InvokeDynamicScanner.QueryContext.STANDARD).isNotNull();
        }

        @Test
        void joinContextExists() {
            assertThat(InvokeDynamicScanner.QueryContext.JOIN).isNotNull();
        }

        @Test
        void groupContextExists() {
            assertThat(InvokeDynamicScanner.QueryContext.GROUP).isNotNull();
        }

        @Test
        void queryContextValuesAreComplete() {
            assertThat(InvokeDynamicScanner.QueryContext.values())
                    .hasSize(3)
                    .containsExactlyInAnyOrder(
                            InvokeDynamicScanner.QueryContext.STANDARD,
                            InvokeDynamicScanner.QueryContext.JOIN,
                            InvokeDynamicScanner.QueryContext.GROUP);
        }
    }

    @Nested
    class LambdaPairRecord {

        @Test
        void lambdaPairStoresMethodNameAndDescriptor() {
            InvokeDynamicScanner.LambdaPair pair =
                    new InvokeDynamicScanner.LambdaPair("lambda$test", "(LTestEntity;)Z");

            assertThat(pair.methodName()).isEqualTo("lambda$test");
            assertThat(pair.descriptor()).isEqualTo("(LTestEntity;)Z");
        }
    }

    @Nested
    class SortLambdaRecord {

        @Test
        void sortLambdaStoresMethodNameDescriptorAndDirection() {
            InvokeDynamicScanner.SortLambda sortLambda =
        ...
        @Test
        void sortLambdaSupportsDescendingDirection() {
            InvokeDynamicScanner.SortLambda sortLambda =
                    new InvokeDynamicScanner.SortLambda(
                            "lambda$sort",
